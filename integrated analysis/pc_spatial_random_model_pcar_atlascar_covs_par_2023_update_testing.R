...
#old version that I'm keeping around until I figure out what is going on

# adjm <- nb2mat(adj, style = 'B')
# 
# adjl <- nb2listw(adj)
# 
# adjg <- graph_from_adjacency_matrix(adjm)
# 
# adjl <- as_adj_list(adjg)
# 
# adj.sparse <- unlist(adjl)
# 
# num <- unlist(lapply(adjl, length))
...
eff.covs <- left_join(eff[, c(1:34, 38)], covs, by = c('PC_ID2'))
...
bba <- read.csv('quad_level_bba2_summary.csv')
bba$QuadName <- str_replace_all(bba$QuadName, ' ', '_')

bba.det <- rep(NA, times = nrow(bba))     #make a vector to house the results
bba.det <- ifelse(bba$Blck.GW >= 1, 1, 0) #summarize the data as detections ignoring NAs for now
bba.det <- data.frame(QuadCode = bba$QuadName, bba.det = bba.det)

bba.br <- rep(NA, times = nrow(bba))
bba.br <- ifelse(bba$Blck.GW > 2, 1, 0)   #up the threshold for inclusion, only probable and greater
bba.br <- data.frame(QuadCode = bba$QuadName, bba.br = bba.br)
...
quad.bba <- left_join(quadmap.dat, bba, by = 'QuadName')

be <- ifelse(quad.bba$Blck.GW > 2, 1, 0)  
be <- ifelse(quad.bba$quad.comp < 1, NA, be)
...
save(out_mcmc, file= paste0('./mcar_results/mebbacar_spp_mpcar_zip_', spp, 'mcar_testing_pois_quadcov_nov_bc_prob_testing2.Rdata'))
